...
void LibrarySystem::registerUser() {
    ...
    if (pwd.length() < 6) {
        cout << "注册失败：密码长度至少需要6位。" << endl;
        cin.ignore(1024, '\n');
        system("pause");
        return;
    }

    // 2. 创建新用户 (默认注册为读者 Role=2)
    userList.push_back(new Reader(acc, pwd));
    
    cout << "注册成功！您现在可以使用该账号登录了。" << endl;
        ...
        switch (choice) {
            case 1: // 登录
                if (login()) {
                    ...
                    currentUser = NULL; // 退出菜单后注销
                }
                break;
                
            case 2: // 注册
                registerUser(); // <--- 【新增】调用注册
                break;
                
            case 0: // 退出
                cout << "谢谢使用，再见！" << endl;
                return; // 结束 run 函数，程序退出
